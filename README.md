...
This could be improved in the future in a number of ways, as described below in the "Further Improvements" section.

## Further Improvements
* Parameterizing the versions of Gradle TestKits would be beneficial.  Currently, the way the tests are executed against multiple versions of Gradle is to run the `integrationTestsAll` task, which executes the integrationTests against all versions of Gradle for a specific major release (currently coded to version '7.x').  Having this improved to run these as parameters to the tests, rather than multiple copies of the task, would be an improvement and more dynamic.
...
* Currently the list of Gradle versions to test against for the integrationTests is hardcoded.  It would be much better to dynamically generate this list from an existing list of all available Gradle versions.  I attempted this by making a request of the Gradle Distributions site, but felt that it was overhandling in the code for the currently requirements.  This would be a great improvement to have this list dynamically generated, so additional versions of Gradle are automatically tested and support.
* * Getting the list of versions from the remote site, and parsing HTML responses is not ideal.  I'm unaware at this time if there is an API to get this data from Gradle, but this was as simple as I could make it right now.  Currently, this is a step above a hard-coded list, as that list may change with time, and would become unmanageable quickly, especially if multiple major versions are supported.
